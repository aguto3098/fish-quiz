(function(){
  ...
  function renderResult(){
    ...
    els.rankLine.textContent = '';
    els.lbTitle.textContent = 'Bestenliste heute · ' + meta.label;
  }
  ...
  function normalizeName(name){
    var n = (name || 'anonym').toLowerCase().trim().replace(/[^a-z0-9äöüàéè_-]+/gi, '-').slice(0, 40);
    return n || 'anonym';
  }

  function docKey(difficulty, name){
    return difficulty + '-' + normalizeName(name);
  }

  function submitScore(){
    if (!db) return;
    var difficulty = state.difficulty;
    var key = docKey(difficulty, state.name);
    var today = todayStr();
    var ref = db.doc('leaderboard/' + key);
    ref.get().then(function(snap){
      if (!snap.exists){
        return ref.set({ name: state.name, score: state.score, difficulty: difficulty, date: today, updatedAt: Date.now() });
      }
      var data = snap.data();
      if (data.date !== today){
        return ref.set({ name: state.name, score: state.score, difficulty: difficulty, date: today, updatedAt: Date.now() });
      ...
      if (state.score > data.score){
        return ref.update({ score: state.score, updatedAt: Date.now() });
      }
    }).then(function(){
      updateRank(difficulty, today);
    }).catch(function(){});
  }

  function updateRank(difficulty, today){
    if (!db) return;
    var myKey = docKey(difficulty, state.name);
    db.collection('leaderboard').where('difficulty', '==', difficulty).where('date', '==', today)
      .orderBy('score', 'desc').limit(10).get().then(function(snap){
        var idx = snap.docs.findIndex(function(d){ return d.id === myKey; });
        if (idx >= 0){
          els.rankLine.innerHTML = 'Heute Platz <strong>' + (idx + 1) + '</strong> von ' + snap.docs.length + ' in ' + DIFFICULTY_META[difficulty].label + '.';
        } else {
          els.rankLine.textContent = 'Noch nicht in den heutigen Top 10 von ' + DIFFICULTY_META[difficulty].label + '.';
        }
      }).catch(function(){});
  }

  function renderLeaderboard(list){
    var myKey = docKey(state.difficulty, state.name);
    ...
    list.forEach(function(entry, i){
      ...
      li.className = 'lb-row' + (entry._id === myKey ? ' me' : '');
      li.innerHTML =
        '<span class="lb-rank">' + (i + 1) + '</span>' +
        '<span class="lb-name">' + (entry.name || 'Anonym') + '</span>' +
        '<span class="lb-score">' + entry.score + '</span>';
      els.lbList.appendChild(li);
    });
  }

  function initLeaderboard(difficulty){
    if (!db) return;
    if (unsubLeaderboard) unsubLeaderboard();
    var today = todayStr();
    var q = db.collection('leaderboard').where('difficulty', '==', difficulty).where('date', '==', today).orderBy('score', 'desc').limit(10);
    unsubLeaderboard = q.onSnapshot(function(snap){
      renderLeaderboard(snap.docs.map(function(d){ var data = d.data(); data._id = d.id; return data; }));
    }, function(){
      els.lbEmpty.hidden = true;
  ...
  // re-subscribe to the right difficulty's board whenever a result screen is about to show
  var _origFinish = finishQuiz;
  finishQuiz = function(){
    _origFinish();
    initLeaderboard(state.difficulty);
  };
  ...
  (function initDb(){
    if (window.claude && typeof window.claude.use === 'function'){
      window.claude.use('db').then(function(ns){
        db = ns;
        if (!db) els.lbHint.hidden = false;
      }).catch(function(){ els.lbHint.hidden = false; });
    } else {
      els.lbHint.hidden = false;
    }
  })();
